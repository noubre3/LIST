...
export default function AddFromPhotoModal({ onClose, onSaved }: Props) {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      setRecipe({ ...data, photo_url: null, source_url: null })
      setStep('review')
    } catch {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="bg-[#2D1468] rounded-t-2xl px-6 py-4 flex items-center justify-between">
          <h2 className="text-[#FFCC00] font-['Bayon'] text-2xl m-0">ADD FROM PHOTO</h2>
        ...
        <div className="p-6">
          ...
          {step === 'input' && (
            <div>
              <p className="text-[#6B6480] font-['Montserrat'] mb-4">
                Take a photo or upload an image of a handwritten or printed recipe. AI will extract it automatically.
              </p>

              {/* Photo preview / upload area */}
              <div
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-[#CC3399] rounded-2xl p-8 text-center cursor-pointer hover:bg-[#FFF0F8] transition-colors mb-4"
              >
                {preview ? (
                  <img src={preview} alt="Recipe" className="max-h-64 mx-auto rounded-xl object-contain" />
                ) : (
                  <div>
                    <div className="text-5xl mb-3">📷</div>
                    <p className="text-[#2D1468] font-['Bayon'] text-xl m-0">TAP TO TAKE OR UPLOAD PHOTO</p>
                    <p className="text-[#6B6480] font-['Montserrat'] text-sm mt-1">JPG, PNG supported</p>
                  </div>
                )}
              </div>
              ...
              {preview && (
                <button
                  ...
                  className="w-full mb-3 border-2 border-[#F0EAF8] text-[#6B6480] font-['Montserrat'] font-bold text-sm uppercase tracking-widest py-2 rounded-full bg-transparent cursor-pointer hover:bg-[#F0EAF8] transition-colors"
                >
              ...
              <button
                ...
                disabled={!preview}
                className="w-full bg-[#CC3399] text-white font-['Montserrat'] font-bold uppercase tracking-widest py-3 rounded-full hover:bg-[#2D1468] transition-colors border-0 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Extract Recipe
              </button>
            </div>
          ...
          {step === 'loading' && (
            <div className="text-center py-12">
              ...
              <p className="text-[#2D1468] font-['Bayon'] text-2xl">READING YOUR RECIPE...</p>
              <p className="text-[#6B6480] font-['Montserrat'] text-sm mt-2">AI is extracting ingredients and instructions</p>
            </div>
          )}
          ...
          {step === 'review' && recipe && (
            <div>
              <p className="text-[#6B6480] font-['Montserrat'] text-sm mb-4">
                Review the extracted recipe and correct anything before saving.
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleSave}
                  className="flex-1 bg-[#2D1468] text-[#FFCC00] font-['Montserrat'] font-bold uppercase tracking-widest px-8 py-3 rounded-full hover:bg-[#CC3399] hover:text-white transition-colors border-0 cursor-pointer"
                >
                  Save Recipe
                </button>
              </div>
